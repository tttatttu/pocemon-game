...
const counterWin = (board, player1, player2) => {
  ...
  board.forEach((item) => {
    if (item.card.position === "red") {
      player1Count++;
    }

    if (item.card.position === "blue") {
      player2Count++;
    }
  });
...
const BoardPage = () => {
  const { pokemons, onGetCardsPlayer } = useContext(
    PokemonsContext
  );
  ...
  // const [onGetCardsPlayer2, setonGetCardsPlayer2] = useState({});

  const history = useHistory();

  // console.log(onGetCardsPlayer2);
  ...
  useEffect(async () => {
    ...
    onGetCardsPlayer(player1, player2Request.data);
  ...
  useEffect(() => {
    if (steps === 9) {
      ...
      return history.push("/game/finish");
    }
  }, [steps]);
